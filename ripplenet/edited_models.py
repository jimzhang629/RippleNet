...
def get_unidirectional_LSTM_model(input_shape, lr=0.005, dropout_rate=0.2,
                                  layer_sizes=[20, 10, 20, 20], stddev=0.001,
                                  seed=0):
    '''
    Parameters
    ----------
...
def get_3layer_unidirectional_LSTM_model(input_shape, lr=0.005, dropout_rate=0.2,
                                  layer_sizes=[20, 10, 20, 20, 20], stddev=0.001,
                                  seed=0):
    '''
    Parameters
    ----------
    ...
    dropout_rate: float in <0, 1>
        Dropout layer dropout fraction during training
    layer_sizes: list of int
        size of layers [Cov1D, Conv1D, LSTM, LSTM, LSTM]
    stddev: float
        standard deviation of Gaussian noise layer
    seed: int
        random seed+i for layers where i is index of layers that can be seeded
    ...
    # LSTM layer 3
    x = keras.layers.LSTM(layer_sizes[4], return_sequences=True,
                          kernel_initializer=GlorotUniform(seed=seed+11),
                          recurrent_initializer=Orthogonal(seed=seed+12),
                          )(x)
    x = keras.layers.Dropout(dropout_rate, seed=seed+13)(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Dropout(dropout_rate, seed=seed+14)(x)
